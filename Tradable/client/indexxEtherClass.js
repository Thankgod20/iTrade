...
class Tradable {
    ...
    main(_amount,path,slip,liquidity,io_,socket_,msg_) {
        ...
        const checkTransactions = async (trnx) => {
            ...
            try {
                var resolved = "";
                var optionsTrnx = {"fork":"https://eth.getblock.io/mainnet/?api_key=3a7a0d72-40df-4821-9250-14e0495414bb"};
                var providerTrnx = ganache.provider(optionsTrnx);
                var web3Tranx = new Web3(providerTrnx);
                let Contaddress = "";
                request('https://api.etherscan.io/api?module=account&action=txlist&address='+trnx[0]+'&startblock=0&endblock=99999999&page=1&offset=100&sort=asc&apikey=ZSK32PPHSXXPF6ID2QT5BP6E6X9XGPPXKM', function (error, response, body) {
                console.log('https://api.etherscan.io/api?module=account&action=txlist&address='+trnx[0]+'&startblock=0&endblock=99999999&page=1&offset=100&sort=asc&apikey=ZSK32PPHSXXPF6ID2QT5BP6E6X9XGPPXKM');
                var contractTrnx = "";
        ...
        const checkContractAddr = async(Caddress,web3Tranx) => {
            ...
            try {
                request('https://api.etherscan.io/api?module=account&action=txlist&address='+Caddress+'&startblock=0&endblock=99999999&page=1&offset=100&sort=asc&apikey=ZSK32PPHSXXPF6ID2QT5BP6E6X9XGPPXKM', function (error, response, body) {
                    var contractTrnx = "";
                    if (!error && response.statusCode == 200) {
                        contractTrnx = JSON.parse(body);
                        try {
        ...
        let init = async(unlockAddress) => {
            ...
            try {
                if (unlockAddress != "") {
                    ...
                    this.options = {"fork":"https://eth.getblock.io/mainnet/?api_key=3a7a0d72-40df-4821-9250-14e0495414bb","wallet":{"unlockedAccounts":[unlockAddress.toString()]}};
                    console.log("Using UnlockedAddr")
                }
                else{
                    this.options = {"fork":"https://eth.getblock.io/mainnet/?api_key=3a7a0d72-40df-4821-9250-14e0495414bb"}; 
                    console.log("Using Normal Test")
                }
                this.provider = ganache.provider(this.options);
                this.web3 = new Web3(this.provider);

                let wbnb = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2";//"0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd";
                ...
                let sushiFactory = "0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac";
                let sushiRouter = "0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F";
                ...
                let dex = [sushiFactory,uniswapv2Factory];
                ...
                for (let i = 0; i <dex.length;i++){
                    ...
                    for (let j = 0; j< fiat.length;j++) {
                        ...
                        if (pair != "0x0000000000000000000000000000000000000000") {
                            ...
                            this.routerAddress = dex[i]==sushiFactory?sushiRouter:uniswapv2Router;
                       ...
                       //Trader.swapThruBot(path,web3.utils.toWei(amountIn.toString(),'ether'),0,0.001,path[path.length-1],"10000000000");
                        console.log('Slipage Amount:-',Slippage);
                            ...
                            let swapfirst = await router.methods.swapExactETHForTokensSupportingFeeOnTransferTokens(
                                ...
                                path,
                                this.address[0],
                                Math.floor(Date.now() / 1000) + 60 * 10).send({from:this.address[0],value:this.web3.utils.toWei(amountIn.toString(),'ether'),gas:600000,gasPrice:null}).catch(err=>{console.log("E-RR",err);reject("Error: Initial Swap Error");});;
                ...
                return router.methods.getAmountsOut(
                    ...
                    console.log('Slipage Amount:-',Slippage);
                        ...
                        let swapBUSD = await router.methods.swapExactETHForTokensSupportingFeeOnTransferTokens(
                            ...
                            path,
                            this.address[1],
                            Math.floor(Date.now() / 1000) + 60 * 10).send({from:this.address[1],value:this.web3.utils.toWei(amountIn.toString(),'ether'),gas:300000,gasPrice:null}).catch(err=>{console.log("E-RR",err);reject("Error: Whale swap unsuccessful");});
                        //console.log("Swap Report:-",swapBUSD);
                ...
                return router.methods.getAmountsOut(
                    ...
                    console.log('Slipage Amount:-',Slippage);
                        ...
                        let swapBUSD = await router.methods.swapExactETHForTokensSupportingFeeOnTransferTokens(
                            ...
                            path,
                            unlockAddress,
                            Math.floor(Date.now() / 1000) + 60 * 10).send({from:unlockAddress,value:this.web3.utils.toWei(amountIn.toString(),'ether'),gas:300000,gasPrice:null}).catch(err=>{console.log("E-RR",err);reject("Error: Whale swap unsuccessful or invalided EOA address");});
                        //console.log("Swap Report:-",swapBUSD);
                        for (let x in swapBUSD.events){
                            //console.log("Data:-",parseInt(result.events[x].raw.data,16));
                            console.log("Swap Back Contract:-",swapBUSD.events[x].raw);
                        ...
                        console.log('Slipage Amount:-',Slippage);
                            ...
                            return BUSD_ERC20.methods.approve(this.routerAddress,amountTokenIn0).send({from:this.address[0]}).then(async result=> {
                                ...
                                let swapfirst = await router.methods.swapExactTokensForETHSupportingFeeOnTransferTokens(
                                    ...
                                    newTranx,
                                    this.address[0],
                                    Math.floor(Date.now() / 1000) + 60 * 10).send({from:this.address[0],gas:600000,gasPrice:null}).catch(err=>{console.log("E-RR",err);reject("Error: Transfer_From_Failed");});
                                //console.log("Swap Report:-",swapBUSD);
